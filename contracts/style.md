...
- Public languages must have their own physical URLs with localized `<html lang>`, `<title>`, description, canonical, Open Graph, Twitter metadata, JSON-LD, reciprocal `hreflang`, and sitemap entries. Route language has precedence over saved language preferences, and language selectors should navigate to the equivalent localized URL instead of only swapping text in place.
